...
// This is important when multiple timekeepers are scanning simultaneously
...
try {
    ...
    // Get employee information with all details from faculty_profiles
    $stmt = $db->prepare("
        SELECT 
            u.id,
            u.first_name,
    ...
    // Station restrictions removed - all stations can scan all employees

    $employeeName = trim($row['first_name'] . ' ' . ($row['middle_name'] ? $row['middle_name'] . ' ' : '') . $row['last_name']);
    
    logAction('QR_SCAN', "Scanned QR code for employee: {$row['employee_id']} ($employeeName)");
    
    // Get timekeeper station ID from saved session data (may be null if in TARF)
    $timekeeperStationId = $sessionData['timekeeper_station_id'] ?? $sessionData['station_id'] ?? null;
    
    // Get today's attendance record if exists (from attendance_logs)
    // Since stations can now scan anyone, just get the most recent attendance for today
    if ($isInTarf || $isHoliday) {
        ...
        // Get most recent attendance for today (any station since restrictions removed)
        $stmt2 = $db->prepare("
            SELECT id, employee_id, log_date, time_in, lunch_out, lunch_in, time_out, ot_in, ot_out, station_id, timekeeper_id, remarks, tarf_id, holiday_id, created_at 
            FROM attendance_logs 
    ...
    // If in TARF but no attendance log exists yet, create it
    if ($isInTarf && !$todayAttendance) {
        ...
        ");
        $remarks = "TARF: " . $tarfTitle;
        $stmtInsert->execute([$row['employee_id'], $today, $timeIn, $lunchOut, $lunchIn, $timeOut, $remarks, $tarfId]);
    ...
    // If it's a holiday but no attendance log exists yet, create it
    if ($isHoliday && !$todayAttendance) {
        // Get Holiday ID
        $holidayId = $holidayData['id'];
        
        // Ensure holiday_id column exists
        try {
        ...
        $stmtInsert->execute([$row['employee_id'], $today, $remarks, $holidayId]);
